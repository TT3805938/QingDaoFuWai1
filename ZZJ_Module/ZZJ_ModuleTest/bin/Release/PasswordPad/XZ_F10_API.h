...
extern "C"
{
	...
	//X9.9算法，但是每步执行3DES算法
	int WINAPI SUNSON_MakeX99ECBMac_3DES(int nMacDataLen,unsigned char* ucMacData,unsigned char *hexReturnInfo);
	//标准的 mac
	...
	//设置终止键
	int WINAPI SUNSON_SetTerminateKey(unsigned char nCancle_TerminateFlag, unsigned char nEnter_TerminateFlag,unsigned char *pReturnInfo);

	//数据加解密操作
	//key_mode :  0x01 主密钥计算
	//            0x02 工作密钥计算
	//EncryptMode:   0x01 加密运算
	//               0x02 解密运算
	//AlgorithmMode:   0x0001 DES ECB运算
	//                 0x0002 DES CBC运算
	//				   0x0003 3DES ECB运算
	//                 0x0004 3DES CBC运算
	int WINAPI	SUNSON_DataOperation(int MasterKeyId,int WorkKeyId, unsigned char Key_mode,unsigned char EncryptMode,int AlgorithmMode,int datalen,unsigned char *data,unsigned char *DataResult);
